...
cases_Dallas <- cases_TX %>% filter(county_name == "Harris County" & state == "TX")
...
mobility_Dallas <- mobility %>% filter(sub_region_1 == "Texas" & sub_region_2 == "Harris County")
mobility_Dallas_2021_01_19 <- mobility_Dallas %>% filter(date == "2021-01-19")
p_mobility <- ggplot(mobility_Dallas, aes(x = date, y = workplaces_percent_change_from_baseline)) +
  geom_line() +
  geom_smooth() +
  #geom_vline(xintercept = as.numeric(as.Date("2021-01-19")), linetype = "dashed", color = "red") + 
  ...
  geom_point(data = mobility_Dallas_2021_01_19, aes(x = date, y = workplaces_percent_change_from_baseline), color = "red", shape = 19, size = 2)  # Add a red marker

p_mobility <- p_mobility +
  geom_point(data = holidays, aes(x = holiday_date, y = mobility_Dallas$workplaces_percent_change_from_baseline[match(holiday_date, mobility_Dallas$date)]), color = "blue", size = 4) +
  geom_text_repel(data = holidays, aes(x = holiday_date, y = mobility_Dallas$workplaces_percent_change_from_baseline[match(holiday_date, mobility_Dallas$date)], label = holiday_name), color = "blue", size = 3, hjust = 2, vjust = 3)
...
p_mobility <- p_mobility + geom_text_repel(data = mobility_Dallas %>% filter(date == "2021-01-19"), aes(x = date, y = workplaces_percent_change_from_baseline, label = paste("Jan 19","Value: ", mobility_Dallas_2021_01_19$workplaces_percent_change_from_baseline)), 
